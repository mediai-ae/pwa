<template>
  <div class="relative" :dir="isRTL ? 'rtl' : 'ltr'" ref="dropdownRef">
    <button @click="menuOpen = !menuOpen" class="flex items-center gap-2 px-2 py-1 rounded-full border border-gray-300 dark:border-gray-700 hover:shadow transition">
      <img
          src="/profile.png"
          alt="Profile"
      ...
      >
        ...
        <button @click="langOpen = !langOpen" class="dropdown-item justify-between">
          ...
          <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" :d="langOpen ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7'" />
          </svg>
        </button>

        <!-- Language Submenu with rectangular flags -->
        <transition name="fade">
